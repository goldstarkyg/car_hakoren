...
@section('head')
    <link href="{{URL::to('/')}}/js/plugins/nvd3/nv.d3.css" rel="stylesheet" type="text/css">
    <script src="https://cdnjs.cloudflare.com/ajax/libs/d3/3.5.2/d3.min.js" charset="utf-8"></script>
    <script src="{{URL::to('/')}}/js/plugins/nvd3/nv.d3.js"></script>
    <script src="{{URL::to('/')}}/js/plugins/nvd3/stream_layers.js"></script>
    ...
    <div>
        <div>
			...
			<div class="row panel panel-default quickcheck" style="margin:5px 5px 20px 5px; background-color: #fff5e7 !important;border-top: 4px #fbb047 solid; font-size: 17px; font-weight:400;">
				<form action="{{URL::to('/')}}/admintop" method="post" class="form-horizontal">
				<div class="col-md-12" style="margin-top:5px; margin-bottom:20px">
					<div class="col-md-9" style="margin-top:5px; text-align:center;">
						<span class="largerfont">クイックレビュー&nbsp;&nbsp;|&nbsp;&nbsp;{{$quick_check['current_date']}}({{$quick_check['current_day']}}) </span>
					</div>
					<div class="col-md-2" style="margin-top: 5px;" >
						<select name="shop_id" id="shop_id" method="post" class="form-control" style="background-color: #fbb047" onchange="this.form.submit()">
							<option value="0" @if($shop_id == '0') selected @endif>全て</option>
				...
				<div class="col-md-6" style="margin-top: 5px;padding-left:50px;padding-bottom:10px">
					<p>本日の見込売上：
						<span class="largerfont">{{number_format($quick_check['expected_sales'])}}</span>円
						&nbsp;&nbsp;(配車 {{$quick_check['depart_count']}}件 / 返車 {{$quick_check['return_count']}}件 )
          				<br/>本日の実際売上：
						<span class="largerfont">
							@if($shop_kind == 'both')
							{{number_format($today_salesmount->all_sales)}}
							@endif
							@if($shop_kind == 'fuku')
								{{number_format($today_salesmount->fuku_sales)}}
							@endif
							@if($shop_kind == 'okina')
								{{number_format($today_salesmount->okina_sales)}}
							@endif
							@if($shop_kind == 'kagoshima')
								{{number_format($today_salesmount->kagoshima_sales)}}
							@endif
						</span>円
						&nbsp;&nbsp;(配車 {{$quick_check['real_depart_count']}}件 / 返車 {{$quick_check['real_return_count']}}件 )
						<br/>
            				{{date('n', strtotime($quick_check['cu_month']))}}
                    月の見込売上： <span class="largerfont">{{number_format($quick_check['month_expected_sales'])}}</span>円<br/>
            				{{date('n', strtotime($quick_check['cu_month']))}}
                    月の実際売上： <span class="largerfont">{{number_format($quick_check['month_real_sales'])}}</span>円　　
					</p>
				</div>
				<div class="col-md-6" style="margin-top: 5px;padding-bottom:10px">
					<p>本日の予約数：
						<span class="largerfont">{{$quick_check['created_count']}}</span>件&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;キャンセル数：<span class="largerfont">{{$quick_check['cancel_count']}}</span>件 <br/>
            {{date('n', strtotime($quick_check['cu_month']))}}月の予約数：
						<span class="largerfont">{{$quick_check['cu_month_depart_count']}}</span>件&nbsp;&nbsp;
						<span class="largerfont"> {{$quick_check['cu_month_usedcar']}} </span>%が埋まっています。
						<br/>{{date('n', strtotime($quick_check['next_month']))}}月の予約数：
						<span class="largerfont">{{$quick_check['next_month_depart_count']}}</span>件&nbsp;&nbsp;
						<span class="largerfont"> {{$quick_check['next_month_usedcar']}} </span>%が埋まっています。
						<br/>{{date('n', strtotime($quick_check['next_next_month']))}}月の予約数：
						<span class="largerfont">{{$quick_check['next_next_month_depart_count']}}</span>件&nbsp;&nbsp;
						<span class="largerfont"> {{$quick_check['next_next_month_usedcar']}} </span>%が埋まっています。
					</p>
				</div>
					{!! csrf_field() !!}
				</form>
			...
			<div class="row">
				<div class="col-md-9">
					<div class="row panel panel-default" style="background-color: #ffffff; margin-left: 5px;margin-bottom: 10px;padding-bottom: 20px;">
						<div>
							<h3><span class="btnlabel">Box 1</span> <span style="padding-left:4px;">{{date('n')}}月の売上・件数</span></h3>
						</div>
					...
					<div class="row panel panel-default" style="background-color: #ffffff; margin-left: 5px;padding-bottom: 20px;">
						<div>
							<h3><span class="btnlabel">Box 2</span> <span style="padding-left:4px;">{{date('n')}}月の獲得予約</span></h3>
						</div>
						<div class="col-md-6" style="padding-left: 0px; padding-right: 0px;">
					...
					<div class="row panel panel-default" style="background-color: #ffffff; margin-left: 5px;">
						<div>
							<h3><span class="btnlabel">{{date('n')}}月</span>&nbsp;&nbsp; Expected Price</h3>
						</div>
						<div class="col-md-6" style="padding-left: 0px; padding-right: 0px;">
					...
					<div class="row panel panel-default" style="background-color: #ffffff;margin-left: 5px; margin-right: 5px;">
						...
						<div style="background-color: #d7efff;">
							<div style="padding: 30px 0px 30px 10px;;">
								...
								<p>全体&nbsp;:&nbsp;&nbsp;{{number_format($today_salesmount->all_sales)}}円&nbsp;/&nbsp;{{number_format($today_salesmount->all_mount)}}件</p>
								<p>福岡&nbsp;:&nbsp;&nbsp;{{number_format($today_salesmount->fuku_sales)}}円&nbsp;/&nbsp;{{number_format($today_salesmount->fuku_mount)}}件&nbsp;({{$today_salesmount->fuku_percent}}%)</p>
								<p>沖縄&nbsp;:&nbsp;&nbsp;{{number_format($today_salesmount->okina_sales)}}円&nbsp;/&nbsp;{{number_format($today_salesmount->okina_mount)}}件&nbsp;({{$today_salesmount->okina_percent}}%)</p>
								<p>鹿児島&nbsp;:&nbsp;&nbsp;{{number_format($today_salesmount->kagoshima_sales)}}円&nbsp;/&nbsp;{{number_format($today_salesmount->kagoshima_mount)}}件&nbsp;({{$today_salesmount->kagoshima_percent}}%)</p>
							</div>
						</div>
						<div >
							<div style="padding: 30px 0px 20px 10px;;">
								<p style="text-align: center; font-size: 16px; font-weight: bold">補償のみの売上</p>
								<p>全体&nbsp;:&nbsp;&nbsp;{{number_format($todayinsurance->all_insurance_price)}}円&nbsp;({{number_format($todayinsurance->all_insurance_mount)}}件)</p>
								<p>福岡&nbsp;:&nbsp;&nbsp;{{number_format($todayinsurance->fuku_insurance_price)}}円&nbsp;({{number_format($todayinsurance->fuku_insurance_mount)}}件)</p>
								<p>沖縄&nbsp;:&nbsp;&nbsp;{{number_format($todayinsurance->okina_insurance_price)}}円&nbsp;({{number_format($todayinsurance->okina_insurance_mount)}}件)</p>
								<p>鹿児島&nbsp;:&nbsp;&nbsp;{{number_format($todayinsurance->kagoshima_insurance_price)}}円&nbsp;({{number_format($todayinsurance->kagoshima_insurance_mount)}}件)</p>
							</div>
						</div>
						<div>
							<div style="padding: 10px 0px 20px 10px;;">
								<p style="text-align: center; font-size: 16px; font-weight: bold" >オプションのみの売上</p>
								<p>全体&nbsp;:&nbsp;&nbsp;{{number_format($todayoption->all_option_price)}}円&nbsp;({{number_format($todayoption->all_option_mount)}}件)</p>
								<p>福岡&nbsp;:&nbsp;&nbsp;{{number_format($todayoption->fuku_option_price)}}円&nbsp;({{number_format($todayoption->fuku_option_mount)}}件)</p>
								<p>沖縄&nbsp;:&nbsp;&nbsp;{{number_format($todayoption->okina_option_price)}}円&nbsp;({{number_format($todayoption->okina_option_mount)}}件)</p>
								<p>鹿児島&nbsp;:&nbsp;&nbsp;{{number_format($todayoption->kagoshima_option_price)}}円&nbsp;({{number_format($todayoption->kagoshima_option_mount)}}件)</p>
							</div>
						</div>
					</div>
				</div>
			...
			<div class="row">
					...
					<div class="panel panel-default">
						<div class="row" style="background-color: #ffffff; margin: 10px 0px 10px 5px;">
							...
							<div style="padding: 10px 0px 0px 10px ;">
								<!--<span>{{$todaybooking['date']}}  </span>-->
								<span>{{$todaybooking['shop_name']}} </span>
							</div>
							...
							<table id="bookinglist" class="table table-borderless data-table" width="100%">
								...
								<tbody>
								<?php $i = 0; ?>
								@foreach($todaybooking['list'] as $book)
								<?php $i++; ?>
										<!---->
								<tr valign="middle">
									<td>
									...
									<td>
									   <span class="po_title" style="background: {{$book->color}}">{{$book->portal_name}}</span>
									</td>
									<td class="cell">
										<span>{{$book->period}}</span>
									</td>
									<td>
										<span>{{$book->class_name}} {{$book->smoke}}</span>
									</td>
									<td>
										<span>{{$book->loop_user}}</span>
									</td>
								</tr>
								@endforeach
								</tbody>
				...
				<div class="col-md-4" >
					<div class="panel panel-default">
						<div>
							<h3><span class="btnlabel">Box 5</span> <span style="padding-left:4px;">{{$quick_check['current_date']}}の予約経路</span></h3>
						</div>
						<div style="background-color: #ffffff;margin: 10px 5px 10px 0px; ">
							<div class="row" style="padding: 10px 0px 0px 10px ;">
								<!-- <span>{{$todayportalbooking['date']}}  </span> -->
								<span>{{$todayportalbooking['shop_name']}} </span>
							</div>
							<div class="row">
								<div class="col-md-4">
									<span style="font-size: 16px;margin-top: 61px;position: absolute;font-weight: bold;">全体 {{$todayportalbooking['all']}}件</span>
								</div>
							...
							<div class="row" style="margin: 0px 5px 10px 10px;">
								...
								@foreach( $portal as $po)
								<div class="col-md-4" style="padding-left:0px; padding-right:5px;">
									<div class="pielabel portal_pie" id="portal_{{$po->id}}">
										<label style="padding:0px;width:5px;margin-top:5px; background-color:{{$po->color}}">&nbsp;</label>
										<label>{{$po->key}}</label>
										<label> {{$po->y}}</label>
									</div>
								</div>
				...
				<div class="col-md-12">
					<div class="row panel panel-default" style="background-color: #ffffff; margin: 0px 5px 10px 5px;">
						<div class="col-md-12" style="padding-left: 0px; padding-right: 0px;">
							<div>
								<h3><span class="btnlabel">Box 6</span> <span style="padding-left:4px;">{{date('Y')}}年の売上推移</span></h3>
							</div>
			...
			<div class="row">
				<div class="col-md-12">
					<div class="row panel panel-default" style="background-color: #ffffff;margin: 0px 5px 10px 5px; ">
						<div class="col-md-12" style="padding-left: 0px; padding-right: 0px;">
							<div>
								<h3><span class="btnlabel">Box 7</span> <span style="padding-left:4px;">{{date('Y')}}年の配車数の推移</span></h3>
							</div>
			...
			<div class="row">
				<div class="col-md-12">
					<div class="row panel panel-default" style="background-color: #ffffff; margin: 0px 5px 10px 5px;">
						<div class="col-md-12" style="padding-left: 0px; padding-right: 0px;">
							<div>
								<h3><span class="btnlabel">Box 8</span> <span style="padding-left:4px;">{{date('Y')}}年の予約件数と経路</span></h3>
							</div>
...
//            ],
		...
		function monthlyBooking() {
			return stream_layers(2,13).map(function(data, i) {
				var key, val , bgcolor;
				if(i == 0) {
					 key = '{{$monthbookingamount[0]->key}}年';
					 val = <?php echo json_encode($monthbookingamount[0]->values); ?> ;
					 bgcolor = '{{$monthbookingamount[0]->color}}';
				}
				if(i == 1) {
					key = '{{$monthbookingamount[1]->key}}年';
					val = <?php echo json_encode($monthbookingamount[1]->values); ?> ;
					bgcolor = '{{$monthbookingamount[1]->color}}';
				}
				return {
					key:   key,
					values: val,
