            ...
            <div class="panel panel-default">
                <div class="row">
                    ...
                    <div class="col-md-10 m-t-sm">
                        ...
                        <form method="POST" name="searchform" id="searchform" action="{{URL::to('/')}}/sales/authmanagement" accept-charset="UTF-8" role="form" class="form-horizontal" enctype="multipart/form-data">
                            {!! csrf_field() !!}
            ...
            <div class="panel panel-default shadow-box book_all_wrap">
                <div class="panel-body">
                    @if($cond == 'one')
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <table class="table table-bordered">
                                    ...
                                    <tr>
                                        <td class="sum_title">{{displayEmpty(number_format($sum))}}円</td>
                                    </tr>
                                </table>
                            </div>
                            ...
                            <div class="col-md-3">
                                <table class="table table-bordered">
                                    <tr>
                                        <th class="sum_title">予約合計金額({{displayEmpty(number_format($reser_price->original_count))}}件)</th>
                                    </tr>
                                    <tr>
                                        <td class="sum_title">{{displayEmpty(number_format($reser_price->price))}}円</td>
                                    </tr>
                                </table>
                            </div>
                            <div class="col-md-1" style="vertical-align: middle">
                            ...
                            <div class="col-md-3">
                                <table class="table table-bordered">
                                    <tr>
                                        <th class="sum_title">予約のキャンセル金額({{displayEmpty(number_format($cancel_price->count))}}件)</th>
                                    </tr>
                                    <tr>
                                        <td class="sum_title">{{displayEmpty(number_format($cancel_price->price))}}円</td>
                                    </tr>
                                </table>
                            </div>
                      </div>
                        ...
                        <div class="row">
                            <h2>
                                予約 {{displayEmpty(number_format($reser_price->original_count))}}件
                                @if($reser_price->additional_count > 0)
                                    追加注文 {{displayEmpty(number_format($reser_price->additional_count))}}件
                                @endif
                            </h2>
                            <table class="table table-bordered" style="width:auto;text-align:center">
                                ...
                                $total_point = 0;
                                ?>
                                @foreach($temporal_list as $de)
                                    <tr>
                                        <td><a href="{{URL::to('/')}}/booking/detail/{{ $de->book_id }}" title="予約詳細を見る">
                                                <span> {{$de->name}} </span>
                                            </a>
                                        </td> <!--id-->
                                        <td>{{$de->user_name}}</td> <!--id-->
                                        <td>
                                        <span class="new_row" >
                                            <label>{{displayEmpty(number_format($de->payment))}}</label>
                                            円
                                        </span>
                                            <span>@if(!empty($de->pay_method))({{$de->pay_method}})@endif</span>
                                        </td> <!--portal payment-->
                                        <td>{{displayEmpty(number_format($de->basic_price))}}</td> <!--basic price-->
                                        <td>{{displayEmpty(number_format($de->insurance1))}}</td> <!--insurance1-->
                                        <td>{{displayEmpty(number_format($de->insurance2))}}</td> <!--insurance2-->
                                        <td>{{displayEmpty(number_format($de->etc))}}</td><!--etc-->
                                        <td>{{displayEmpty(number_format($de->baby_seat))}}</td><!--bayby seat-->
                                        <td>{{displayEmpty(number_format($de->child_seat))}}</td><!--child seat-->
                                        <td>{{displayEmpty(number_format($de->junior_seat))}}</td><!--junior seat-->
                                        <td>{{displayEmpty(number_format($de->snow_type))}}</td> <!--snow type-->
                                        <td>{{displayEmpty(number_format($de->smart_return))}}</td> <!--drive smart out-->
                                        <!--<td>{{displayEmpty(number_format($de->hotel_return))}}</td>--> <!--hote lreturn-->
                                        <td>{{displayEmpty(number_format($de->adjustment_price))}}</td> <!--adjust price-->
                                        <td>{{displayEmpty(number_format($de->extend_price))}}</td> <!--extend night-->
                                        <td>{{displayEmpty(number_format($de->point))}}</td> <!--point-->
                                    </tr>
                                    <?php
                                    $count++;
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="new_row" >
                                            <label>{{displayEmpty(number_format($total_payment))}}</label>
                                            円
                                        </span>
                                    </td> <!--portal payment-->
                                    <td>{{displayEmpty(number_format($total_basic_price))}}</td> <!--basic price-->
                                    <td colspan="2" style="text-align: center">{{displayEmpty(number_format($total_insurance1+$total_insurance2))}}</td> <!--insurance1-->
                                    <td colspan="6" style="text-align: center">{{displayEmpty(number_format($total_etc + $total_baby_seat +$total_child_seat + $total_junior_seat + $total_snow_type + $total_smart_return + $total_hotel_return))}}</td><!--etc-->
                                    <td>{{displayEmpty(number_format($total_adjustment_price))}}</td> <!--adjust price-->
                                    <td>{{displayEmpty(number_format($total_extend_price))}}</td> <!--extend night-->
                                    <td>{{displayEmpty(number_format($total_point))}}</td> <!--point-->
                                </tr>
                                <tr>
                                    <td colspan="2">合計</td>
                                    <td colspan="13">{{displayEmpty(number_format($total_payment+ $total_point))}}</td>
                                </tr>
                                </tbody>
                            </table>
                        </div>
                        ...
                        <!--start cancellatiion-->
                        <div class="row">
                        <h2>キャンセル予約　{{displayEmpty(number_format($cancel_price->count))}}件</h2>
                        <table class="table table-bordered" style="width:auto;text-align:center">
                            <thead>
                            <tr>
                              <th style="width:100px;text-align:center">ID</th> <!--id-->
                            ...
                            $total_point = 0;
                            ?>
                            @foreach($canceltemporal_list as $de)
                                <tr>
                                    <td><a href="{{URL::to('/')}}/booking/detail/{{ $de->book_id }}" title="予約詳細を見る">
                                            <span> {{$de->name}} </span>
                                        </a>
                   ...
                   @if($cond == 'day')
                        ...
                        <table class="table table-bordered" style="text-align:center">
                            ...
                            @foreach($result_list as $sum)
                                ...
                                <tr>
                                    <td>{{$sum->day}}</td>
                                    <td>{{displayEmpty(number_format($sum->fuku_reservation_number))}}</td>
                                    <td>{{displayEmpty(number_format($sum->fuku_cancel_number))}}</td>
                                    <td>{{displayEmpty(number_format($fuku_price))}}</td>
                                    <td>{{displayEmpty(number_format($sum->okina_reservation_number))}}</td>
                                    <td>{{displayEmpty(number_format($sum->okina_cancel_number))}}</td>
                                    <td>{{displayEmpty(number_format($okina_price))}}</td>
                                    <td>{{displayEmpty(number_format($sum->kagoshima_reservation_number))}}</td>
                                    <td>{{displayEmpty(number_format($sum->kagoshima_cancel_number))}}</td>
                                    <td>{{displayEmpty(number_format($kagoshima_price))}}</td>
                                    <td>{{displayEmpty(number_format($all_reservation_number))}}</td>
                                    <td>{{displayEmpty(number_format($all_cancel_number))}}</td>
                                    <td>{{displayEmpty(number_format($all_price))}}</td>
                                </tr>
                            @endforeach
                                <tr>
                                    <td>小計</td>
                                    <td>{{displayEmpty(number_format($fuku_reservation_number_total))}}</td>
                                    <td>{{displayEmpty(number_format($fuku_cancel_number_total))}}</td>
                                    <td>{{displayEmpty(number_format($fuku_price_total))}}</td>
                                    <td>{{displayEmpty(number_format($okina_reservation_number_total))}}</td>
                                    <td>{{displayEmpty(number_format($okina_cancel_number_total))}}</td>
                                    <td>{{displayEmpty(number_format($okina_price_total))}}</td>
                                    <td>{{displayEmpty(number_format($kagoshima_reservation_number_total))}}</td>
                                    <td>{{displayEmpty(number_format($kagoshima_cancel_number_total))}}</td>
                                    <td>{{displayEmpty(number_format($kagoshima_price_total))}}</td>
                                    <td>{{displayEmpty(number_format($all_reservation_number_total))}}</td>
                                    <td>{{displayEmpty(number_format($all_cancel_number_total))}}</td>
                                    <td>{{displayEmpty(number_format($all_price_total))}}</td>
                                </tr>
                                <tr>
                                    <td>合計</td>
                                    <td colspan="12">{{displayEmpty(number_format($fuku_price_total+$okina_price_total + $kagoshima_price_total))}}</td>
                                </tr>
                            </tbody>
                        </table>
                        </div>
