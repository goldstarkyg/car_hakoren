...
class Complex {
	...
	public static function _parseComplex($complexNumber)
	{
		...
		$validComplex = preg_match('/^([\-\+]?(\d+\.?\d*|\d*\.?\d+)([Ee][\-\+]?[0-2]?\d{1,3})?)([\-\+]?(\d+\.?\d*|\d*\.?\d+)([Ee][\-\+]?[0-2]?\d{1,3})?)?(([\-\+]?)([ij]?))$/jquery-ui',$complexNumber,$complexParts);

		if (!$validComplex) {
			//	Neither real nor imaginary part, so test to see if we actually have a suffix
			$validComplex = preg_match('/^([\-\+]?)([ij])$/jquery-ui',$complexNumber,$complexParts);
			if (!$validComplex) {
