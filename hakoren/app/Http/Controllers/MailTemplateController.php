...
class MailTemplateController extends Controller
{
    ...
    public function index(){
        ...
        return view('pages.admin.mailtemplate.index', compact('templates'));
    }

    public function edit($tempid){
        $template = \DB::table('mail_templates as t')
            ->select('t.*')->where('t.id', $tempid)->first();
        return view('pages.admin.mailtemplate.edit', compact('template'));
    }

    public function create(){
        return view('pages.admin.mailtemplate.new');
    }
...
//                    curl_setopt($ch[0], CURLOPT_SSL_VERIFYPEER, 0);
            ...
            return Redirect::back()->with('testmailsuccess', 'ご入力のメールアドレスにテストメールを送りました。');
        }
        return redirect('/mailtemplate/')->with('success', '変更を保存しました。');
    }
